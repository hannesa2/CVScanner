...
public class CVProcessor {
    ...
    public static Rect detectBorder(Mat original) {
        ...
        Timber.d("1 original: " + src.toString());
        ...
        Timber.d("4 --> Addition done. sum: " + sum_img.toString());
        ...
        Timber.d("8.3 --> Copy sub matrix done. row: " + row_sub2.toString());
        ...
        Timber.d("9.1 --> Copy sub matrix done. col: " + col_sub.toString());
        ...
        Timber.d("9.3 --> Copy sub matrix done. col: " + col_sub2.toString());
        ...
        if (lines.rows() >= 3) {
            ...
            if (hLines.size() >= 2 && vLines.size() >= 2) {
                ...
                if ((nvLines.size() > 1 && nhLines.size() > 0) || (nvLines.size() > 0 && nhLines.size() > 1)) {
                    ...
                    for (Line l : nvLines) {
                        if (l.length() / frameHeight < requiredCoverageRatio || (left != null && right != null))
                            break;
                    ...
                    for (Line l : nhLines) {
                        if (l.length() / frameWidth < requiredCoverageRatio || (top != null && bottom != null))
                            break;
    ...
    static public Quadrilateral getQuadrilateral(List<MatOfPoint> contours, Size srcSize) {
        ...
        for (MatOfPoint c : contours) {
            ...
            Timber.d("SCANNER approx size: " + points.length);
            ...
            if (points.length == 4) {
                ...
                if (isInside(foundPoints, size) && isLargeEnough(foundPoints, size, 0.25)) {
                    ...
                    Timber.d("SCANNER Not inside defined area");
                }
            }
        }
                        ...
                        if (box1.size.width > 0 && box2.size.width > 0 && 0.5 < ar && ar < 2.0) {
                            if (Math.abs(box1.angle - box2.angle) <= 0.1 ||
                                ...
                                if (distance < (1.5 * (box1.size.height + box2.size.height))) {
                                    ...
                                    Timber.d("SCANNER after merge approx size: " + allPoints.length);
